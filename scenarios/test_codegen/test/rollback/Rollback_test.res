  ...
  Async.it("Rolls back SET -> DELETE -> SET to the deleted state", async t => {
    ...
    await MockIndexer.Indexer.run(
      ...
      async indexerMock => {
        ...
        let historyWaitTimeout = Promise.make(
          (resolve, _reject) => {
      ...
      await MockIndexer.Indexer.run(
        ...
        async indexerMock => {
          ...
          t.expect(
            await Promise.all2((
              (
                indexerMock.query("EntityWithBigDecimal"): promise<
                  array<Indexer.Entities.EntityWithBigDecimal.t>,
                >
              ),
              (
                indexerMock.queryHistory("EntityWithBigDecimal"): promise<
                  array<Change.t<Indexer.Entities.EntityWithBigDecimal.t>>,
                >
              ),
            )),
  ...
  Async.it("Should NOT have duplicate queries after rollback with chunked partitions", async t => {
    ...
    await MockIndexer.Indexer.run(
      ...
      async indexerMock => {
        ...
        let payloads = sourceMock.getItemsOrThrowCalls->Array.map(c => c.payload)
        let gapFills =
          payloads->Array.filter(p => p["fromBlock"] == 116 && p["toBlock"] == Some(118))
        t.expect(
    ...
    async t => {
      ...
      await MockIndexer.Indexer.run(
        ...
        async indexerMock => {
          ...
          | None =>
            JsError.throwWithMessage(
              "Should have a pending continuation call with fromBlock == 116",
            )
          }
