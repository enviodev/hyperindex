...
describe("E2E tests", () => {
  ...
  Async.it("Populates config addresses on init and preserves them across restart", async t => {
      ...
      await MockIndexer.Indexer.run(
        ...
        async indexerMock => {
          ...
          let _ = await Promise.all2((
            MockIndexer.Helper.initialEnterReorgThreshold(
              ~t,
              ~indexerMock,
    ...
    async t => {
      ...
      await MockIndexer.Indexer.run(
        ...
        async indexerMock => {
          ...
          t.expect((
            await indexerMock.queryEffectCache(
              chainScopedEffect,
              ~scope=Chain(1337->ChainId.fromInt),
            ),
            await indexerMock.metric("envio_effect_cache"),
          )).toEqual((
  ...
  Async.it("Effect rate limiting with single call per window", async t => {
    ...
    await MockIndexer.Indexer.run(
      ...
      async indexerMock => {
        ...
        t.expect(executionOrder->Array.length, ~message="should have executed all 4 calls").toEqual(
          4,
        )
      ...
      async indexerMock => {
        ...
        let shrunkChunks =
          sourceMock.getItemsOrThrowCalls->Array.filter(
            c =>
              c.payload["toBlock"]->Option.map(tb => tb - c.payload["fromBlock"] + 1) == Some(180),
          )
        t.expect(
        ...
        sourceMock.resolveGetItemsOrThrow(
          [
            ...Array.fromInitializer(
              ~length=100,
              (i): MockIndexer.Source.itemMock => {blockNumber: 3000 + i, logIndex: 0},
            ),
            {
              blockNumber: 5000,
              logIndex: 0,
      ...
      await MockIndexer.Indexer.run(
        ...
        async indexerMock => {
          ...
          for _ in 0 to 4 {
            ...
            call.resolve(
              [
                {blockNumber: fromBlock + 20, logIndex: 0},
                {blockNumber: fromBlock + 60, logIndex: 0},
              ],
