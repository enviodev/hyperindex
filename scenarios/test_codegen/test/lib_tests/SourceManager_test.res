...
describe("SourceManager fetchNext", () => {
  ...
  Async.it(
    ...
    async t => {
      let sourceManager = SourceManager.make(~isLive=false, ~sources=[source], ~maxPartitionConcurrency=2)
  ...
  Async.it("Can add new partitions until the concurrency limit reached", async t => {
    let sourceManager = SourceManager.make(~isLive=false, ~sources=[source], ~maxPartitionConcurrency=3)
  ...
  Async.it("Sorts after all the filtering is applied", async t => {
    let sourceManager = SourceManager.make(~isLive=false, ~sources=[source], ~maxPartitionConcurrency=1)
  ...
  Async.it(
    ...
    async t => {
      ...
      liveMock0.getItemsOrThrowCalls->Js.Array2.forEach(call =>
        call.reject(
          Source.GetItemsError(UnsupportedSelection({message: "test disable"})),
        )
      )
      // liveMock1 should be the next source (still Live, still Primary)
      await Utils.delay(0)
      liveMock1.getItemsOrThrowCalls->Js.Array2.forEach(call =>
        call.resolve([])
      )
      let _ = await p1
      t.expect(
      ...
      let p2 = sourceManager->SourceManager.executeQuery(~query=mockQuery(), ~isLive=true, ~knownHeight=100)
      liveMock1.getItemsOrThrowCalls->Js.Array2.forEach(call =>
        call.reject(
          Source.GetItemsError(UnsupportedSelection({message: "test disable"})),
        )
  ...
  Async.it(
    ...
    async t => {
      ...
      // Should prefer syncMock1, not switch to syncMock0.
      {
        ...
        | [call] => call.resolve([])
        | _ => Js.Exn.raiseError("Expected query to go to syncMock1 (activeSource)")
        }
        let _ = await p
      }
