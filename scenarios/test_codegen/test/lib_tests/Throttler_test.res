...
describe("Throttler", () => {
  Async.it("Schedules and throttles functions as expected", async () => {
    ...
    Assert.deepEqual(actionsCalled, [1], ~message="Should still be called once before interval elapses")
  ...
  Async.it("Does not run until previous task is finished", async () => {
    ...
    throttler->Throttler.schedule(
      async () => {
        await Time.resolvePromiseAfterDelay(~delayMilliseconds=80)
  ...
  Async.it(
    ...
    async () => {
      ...
      throttler->Throttler.schedule(
        async () => {
          await Time.resolvePromiseAfterDelay(~delayMilliseconds=20)
          actionsCalled->Js.Array2.push(1)->ignore
        },
      )
      throttler->Throttler.schedule(
      ...
      Assert.deepEqual(
        actionsCalled,
        [1, 2],
        ~message="Second action should have been called after the interval has passed",
      )
    },
  )
})
