...
let getAllMockEntity = sql =>
  sql
  ->Postgres.unsafe(`SELECT * FROM "${TestEntity.table.tableName}"`)
  ->Promise.thenResolve(json => json->S.parseOrRaiseWith(TestEntity.rowsSchema))

let getAllMockEntityHistory = sql =>
  sql->Postgres.unsafe(`SELECT * FROM "${TestEntity.entityHistory.table.tableName}"`)
...
describe("Entity History Codegen", () => {
  it("Creates a postgres insert function", () => {
    let expected = `CREATE OR REPLACE FUNCTION "insert_TestEntity_history"(history_row "TestEntity_history", should_copy_current_entity BOOLEAN)
      RETURNS void AS $$
      DECLARE
      ...
      BEGIN
        ...
        IF history_row.previous_entity_history_block_timestamp IS NULL OR history_row.previous_entity_history_chain_id IS NULL OR history_row.previous_entity_history_block_number IS NULL OR history_row.previous_entity_history_log_index IS NULL THEN
          ...
          IF FOUND THEN
            ...
            SELECT "id", "fieldA", "fieldB" FROM "TestEntity" WHERE id = history_row.id INTO v_origin_record;
            IF FOUND THEN
              INSERT INTO "TestEntity_history" (entity_history_block_timestamp, entity_history_chain_id, entity_history_block_number, entity_history_log_index, "id", "fieldA", "fieldB", "action")
