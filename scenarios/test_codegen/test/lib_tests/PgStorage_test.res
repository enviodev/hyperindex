...
describe("PgStorage.makeStorageFromEnv ClickHouse env var validation", () => {
  ...
  Async.it(
    ...
    async t => {
      let config = {
        ...MockIndexer.config,
        storage: ({postgres: true, clickhouse: true, duckdb: false, ducklake: false}: Config.storage),
      }
  ...
  Async.it(
    ...
    async t => {
      let config = {
        ...MockIndexer.config,
        storage: ({postgres: true, clickhouse: false, duckdb: false, ducklake: false}: Config.storage),
      }
