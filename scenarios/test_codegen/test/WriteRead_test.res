...
describe("Write/read tests", () => {
  Async.it("Test writing and reading entities with special cases", async t => {
    ...
    await MockIndexer.Indexer.run(
      ...
      async indexerMock => {
        ...
        t.expect(
          await (
            indexerMock.query("EntityWithAllTypes"): promise<
              array<Indexer.Entities.EntityWithAllTypes.t>,
            >
          ),
        ).toEqual([entityWithAllTypes])
        t.expect(
          await (
            indexerMock.queryHistory("EntityWithAllTypes"): promise<
              array<Change.t<Indexer.Entities.EntityWithAllTypes.t>>,
            >
          ),
        ).toEqual([
          Set({
        ...
        t.expect(
          await (
            indexerMock.query("EntityWithAllNonArrayTypes"): promise<
              array<Indexer.Entities.EntityWithAllNonArrayTypes.t>,
            >
          ),
        ).toEqual([entityWithAllNonArrayTypes])
        t.expect(
          await (
            indexerMock.queryHistory("EntityWithAllNonArrayTypes"): promise<
              array<Change.t<Indexer.Entities.EntityWithAllNonArrayTypes.t>>,
            >
          ),
        ).toEqual([
          Set({
        ...
        t.expect(
          await (
            indexerMock.query(
              ...
              array<
                Indexer.Entities.EntityWith63LenghtName______________________________________one.t,
              >,
            >
          ),
        ).toEqual([
        ...
        t.expect(
          await (
            indexerMock.query(
              ...
              array<
                Indexer.Entities.EntityWith63LenghtName______________________________________two.t,
              >,
            >
          ),
        ).toEqual([
  ...
  Async.it("Keeps committed entities across batches without rewriting their history", async t => {
    let sourceMock = MockIndexer.Source.make(~chainId=#1337, [#getHeightOrThrow, #getItemsOrThrow])
    await MockIndexer.Indexer.run(
      ~chains=[{chain: #1337, sourceConfig: Config.CustomSources([sourceMock.source])}],
      ~saveFullHistory=true,
        ...
        sourceMock.resolveGetItemsOrThrow([
          {
            ...
            handler: async ({context}) => {
              ...
              whereEqTokenIdTest :=
                (await context.\"Token".getWhere({tokenId: {_eq: BigInt.fromInt(50)}}))
