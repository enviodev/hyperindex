...
describe("Hash", () => {
  ...
  it("set", () => {
    Assert.throws(
      ...
      ~error={
        "message": `Failed to get hash for Set. If you're using a custom rescript-schema schema make it based on the string type with a decoder: const myTypeSchema = S.transform(S.string, undefined, (yourType) => yourType.toString())`,
      },
    )
  })
