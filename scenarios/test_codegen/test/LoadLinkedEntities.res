...
//   t->Assert.deepEqual(.optA1, Some(a1), ~message="Incorrect entity loaded")
//
...
//
//   t->Assert.deepEqual(.b1.id, a1.b_id, ~message="b1.id should equal testingA.b_id")
//
...
//   let testingA = handlerContext.a.all
//
//   t->Assert.deepEqual(.[Some(a1)], testingA, ~message="testingA should have correct entities")
//
//   let optA1 = testingA->Belt.Array.getUnsafe(0)
//   t->Assert.deepEqual(.optA1, Some(a1), ~message="Incorrect entity loaded")
//
...
asyncTest("Async linked entity loaders: should update the big int to be the same ", async (. t) => {
  ...
  let stringInAFromC =
    updatedMockDb.entities.a.get(EventHandlers.aIdWithGrandChildC)->Belt.Option.flatMap(a =>
      a.optionalStringToTestLinkedEntities
    )
  t->Assert.deepEqual(. stringInAFromC, Some(messageFromC))

  // Expected string to be null still since no c grandchild.
  let optionalStringToTestLinkedEntitiesNoGrandchild =
    updatedMockDb.entities.a.get(EventHandlers.aIdWithNoGrandChildC)->Belt.Option.flatMap(a =>
      a.optionalStringToTestLinkedEntities
    )
  Js.log(optionalStringToTestLinkedEntitiesNoGrandchild)
  t->Assert.deepEqual(. optionalStringToTestLinkedEntitiesNoGrandchild, None)
})
