...
describe("Chains State", () => {
  ...
  describe("chains dict", () => {
    it("should support multiple chains with different states", () => {
  ...
  describe("chains in context", () => {
    Async.it("should be accessible in handler context", async () => {
      ...
      Assert.equal(handlerContext.chains->Js.Dict.get("1")->Belt.Option.map(c => c.isReady), Some(false))
    })
  })
})
