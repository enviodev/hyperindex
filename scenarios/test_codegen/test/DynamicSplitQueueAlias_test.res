...
describe("Dynamic-registration partition split queue aliasing", () => {
  Async.it("does not skip a range for the split-off contract", async t => {
    let processed = []
    let record = id => (async _ => processed->Array.push(id)->ignore)->Obj.magic

    let gravatarAddress = "0x2B2f78c5BF6D9C12Ee1225D5F374aa91204580c3"->Address.Evm.fromStringOrThrow
    ...
    let itemsFor = (call: MockIndexer.Source.getItemsOrThrowCall, ~toBlock) => {
      let addresses = call.payload->MockIndexer.Source.CallPayload.addresses
      worldEvents->Array.filterMap(((address, blockNumber, logIndex, id)) =>
        if (
          ...
          Some(
            ({blockNumber, logIndex, handler: record(id)}: MockIndexer.Source.itemMock),
          )
        } else {
          None
        }
    ...
    let resolveTo = (call: MockIndexer.Source.getItemsOrThrowCall, toBlock) =>
    ...
    let find = (p, ~fromBlock) =>
      ...
      ->Option.getOrThrow(~message=`Expected a pending query for partition ${p} from block ${fromBlock->Int.toString}`)
    ...
    find("0", ~fromBlock=1).resolve(
      [
        ...Array.fromInitializer(~length=100, i => (
          {blockNumber: 2 + mod(i, 8), logIndex: i / 8}: MockIndexer.Source.itemMock
        )),
        {
          blockNumber: 10,
          logIndex: 0,
    ...
    t.expect(
      ...
      ->Array.map(c => (
        c.payload["p"],
        c.payload["fromBlock"],
        (c.payload->MockIndexer.Source.CallPayload.addresses)->Array.length,
      ))
      ->Array.toSorted(((_, a, _), (_, b, _)) => Int.compare(a, b)),
      ~message="the registration splits partition '0' into Gravatar ('0') and NftFactory ('3')",
    ).toEqual([("4", 20_050, 1), ("0", 24_001, 1), ("3", 24_001, 2), ("2", 45_001, 1), ("2", 80_985, 1)])
