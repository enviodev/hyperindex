...
describe("Dynamic-registration partition split queue aliasing", () => {
  Async.it("does not skip a range for the split-off contract", async t => {
    ...
    let itemsFor = (call: MockIndexer.Source.getItemsOrThrowCall, ~toBlock) => {
      ...
      worldEvents->Array.filterMap(
        ((address, blockNumber, logIndex, id)) =>
          if (
            addresses->Array.includes(address) &&
            call.payload["fromBlock"] <= blockNumber &&
            blockNumber <= toBlock
          ) {
            Some(({blockNumber, logIndex, handler: record(id)}: MockIndexer.Source.itemMock))
          } else {
            None
          },
      )
        ...
        find("0", ~fromBlock=1).resolve(
          [
            ...Array.fromInitializer(
              ~length=100,
              (i): MockIndexer.Source.itemMock => {blockNumber: 2 + mod(i, 8), logIndex: i / 8},
            ),
            {
              blockNumber: 10,
              logIndex: 0,
