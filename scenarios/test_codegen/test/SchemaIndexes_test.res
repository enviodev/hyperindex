...
let findIndexes = async (~tableName, ~columns, ~pgSchema) => {
...
let readyAtRows = async (~pgSchema) => {
  let rows: array<{
    "id": ChainId.t,
    ...
    async t => {
      ...
      await MockIndexer.Indexer.run(
        ...
        async indexerMock => {
          ...
          t.expect(
            (
              await findIndexes(~tableName="A", ~columns=["b_id"], ~pgSchema),
          ...
          t.expect(
            (
              (await findIndexes(~tableName="A", ~columns=["b_id"], ~pgSchema))->Array.map(
          ...
          let indexesBeforeRestart = await indexNames(~pgSchema)
          let restarted = await indexerMock.restart()
          await restarted.waitUntilIdle()

          t.expect(
            (await indexNames(~pgSchema), await readyAtByChainId(~pgSchema)),
  ...
  Async.it("Wait for every chain before finalizing, and stamp them together", async t => {
    ...
    await MockIndexer.Indexer.run(
      ...
      async indexerMock => {
        ...
        t.expect(
          (finalizeCalls.contents, await readyAtByChainId(~pgSchema)),
        ...
        let readyAtTimes = await readyAtTimesByChainId(~pgSchema)
  ...
  Async.it("Reach ready even when another table holds the legacy index name", async t => {
    ...
    await MockIndexer.Indexer.run(
      ...
      async indexerMock => {
        ...
        let conflicting = await findIndexes(~tableName="B", ~columns=["c_id"], ~pgSchema)
        let aIndexes = await findIndexes(~tableName="A", ~columns=["b_id"], ~pgSchema)
  ...
  Async.it("Are built mid-backfill by the first query that needs one", async t => {
    ...
    await MockIndexer.Indexer.run(
      ...
      async indexerMock => {
        ...
        t.expect(
          await findIndexes(~tableName="A", ~columns=[optionalColumn], ~pgSchema),
        ...
        t.expect(
          (
            matched.contents,
            (await findIndexes(~tableName="A", ~columns=[optionalColumn], ~pgSchema))->Array.map(
        ...
        t.expect(
          (
            (await findIndexes(~tableName="A", ~columns=[optionalColumn], ~pgSchema))->Array.length,
            (await findIndexes(~tableName="A", ~columns=["b_id"], ~pgSchema))->Array.map(
              entry => entry.name,
            ),
            await readyAtByChainId(~pgSchema),
