...
describe("PIN: multichain indexer enters the reorg threshold", () => {
    ...
    async t => {
      ...
      await MockIndexer.Indexer.run(
        ~chains=[
          {
            chain: #100,
            sourceConfig: Config.CustomSources([chainA.source]),
            maxReorgDepth: 200,
            blockLag: 0,
          },
          {
            chain: #1337,
            sourceConfig: Config.CustomSources([chainB.source]),
            maxReorgDepth: 200,
            blockLag: 0,
          },
        ],
  ...
  Async.it("enters while still within the configured tolerance of the lagged head", async t => {
    ...
    await MockIndexer.Indexer.run(
      ~chains=[
        {
          chain: #1337,
          sourceConfig: Config.CustomSources([source.source]),
          maxReorgDepth: 200,
          blockLag: 0,
        },
      ],
