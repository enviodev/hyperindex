    ...
    pub fn try_from_config_field_selection(
        ...
        for block_field in block_fields {
            let data_type = match block_field {
                ...
                Block::Difficulty => option(Res::BigInt),
                Block::TotalDifficulty => option(Res::BigInt),
                Block::ExtraData => Res::String,
                Block::Size => Res::BigInt,
                Block::GasLimit => Res::BigInt,
                Block::GasUsed => Res::BigInt,
                Block::Uncles => option(array(Res::String)),
                Block::BaseFeePerGas => option(Res::BigInt),
                Block::BlobGasUsed => option(Res::BigInt),
                Block::ExcessBlobGas => option(Res::BigInt),
                Block::ParentBeaconBlockRoot => option(Res::String),
                Block::WithdrawalsRoot => option(Res::String),
                // Block::Withdrawals => todo!(), //should be array of withdrawal record
                Block::L1BlockNumber => option(Res::Int),
                Block::SendCount => option(Res::String),
                Block::SendRoot => option(Res::String),
                Block::MixHash => option(Res::String),
            };
        ...
        for transaction_field in transaction_fields {
            let data_type = match transaction_field {
                ...
                Tx::Hash => Res::String,
                Tx::From => option(Res::Address),
                Tx::To => option(Res::Address),
                Tx::Gas => Res::BigInt,
                Tx::GasPrice => option(Res::BigInt),
                Tx::MaxPriorityFeePerGas => option(Res::BigInt),
                Tx::MaxFeePerGas => option(Res::BigInt),
                ...
                Tx::Nonce => Res::BigInt,
                Tx::Value => Res::BigInt,
                Tx::V => option(Res::String),
                Tx::R => option(Res::String),
                Tx::S => option(Res::String),
                Tx::ContractAddress => option(Res::Address),
                Tx::LogsBloom => Res::String,
                Tx::Root => option(Res::String),
                Tx::Status => option(Res::Int),
                Tx::YParity => option(Res::String),
                Tx::ChainId => option(Res::Int),
                // TxField::AccessList => todo!(),
                Tx::MaxFeePerBlobGas => option(Res::BigInt),
                Tx::BlobVersionedHashes => option(array(Res::String)),
                Tx::Kind => option(Res::Int),
                Tx::L1Fee => option(Res::BigInt),
                Tx::L1GasPrice => option(Res::BigInt),
                Tx::L1GasUsed => option(Res::BigInt),
                Tx::L1FeeScalar => option(Res::Float),
                Tx::GasUsedForL1 => option(Res::BigInt),
            };
