...
let makeBatchSet = (~table: Table.table, ~rowsSchema: S.schema<array<'entityRow>>) =>
  async (sql: Postgres.sql, entities: array<'entityRow>, ~logger=?) => {
    switch entities->S.reverseConvertToJsonWith(rowsSchema) {
    | exception exn =>
      exn->ErrorHandling.mkLogAndRaise(
        ~logger?,
...
let makeWhereEq = (
  ...
  async (
    ...
    let value = try fieldValue->S.reverseConvertToJsonWith(fieldValueSchema) catch {
    | exn => exn->ErrorHandling.mkLogAndRaise(~logger, ~msg=`Failed to serialize value`)
    }
