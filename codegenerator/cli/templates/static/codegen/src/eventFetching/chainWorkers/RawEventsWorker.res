...
module PreviousDynamicContractAddresses: {
  type registration = {address: Ethers.ethAddress, eventId: BigInt.t}
  type matchingRegistration = SameOrLater(registration) | Earlier(registration)
  type t
  let make: unit => t
  let add: (t, registration) => unit
  let getRegistration: (t, Ethers.ethAddress, BigInt.t) => option<matchingRegistration>
...
type rec t = {
  ...
  mutable latestFetchedEventId: promise<BigInt.t>,
