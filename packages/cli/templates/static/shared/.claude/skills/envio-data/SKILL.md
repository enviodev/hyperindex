...
name: envio-data
description: >-
  Use when you need raw HyperSync data — block ranges, event topics, contract
...
```
envio data <field>... --chain=<id|name> [--where="<yaml>"]
```
...
- **--chain** is a numeric chain id (e.g. `8453`), a kebab-case name (e.g.
  ...
  supported yet.
- **--where** is YAML by default; values starting with `{` or `[` are parsed
  as JSON. The schema mirrors indexer filters: `block.number._gte/_lte/_gt/_lt`
  for the block range (Fuel: `block.height`), and per-section field
  constraints that accept a scalar, an array, `{_eq: ...}`, or `{_in: [...]}`.
...
The first row is the earliest matching block. Use `--where="block: number: {_gte: <next_block>}"` from the printed hint to page forward; pick a tight range (50–200 blocks) for fast, deterministic tests.
...
Hits `/height` directly. Useful for picking a recent block range.
...
| `block.number._lte: 2000`      | inclusive upper bound → `to_block` (exclusive translated) |
...
- `knownHeight` mixed with other fields appends an `archiveHeight` block to
  the output; alone, it skips `/query` and hits `/height`.

Full HyperSync query reference:
