...
pub async fn run_init_args(
    ...
    let tests_passed =
        match commands::pm::run_script(pm, "test", &parsed_project_paths.project_root).await {
            Ok(()) => {
                println!("Tests passed — your indexer is ready to go!");
                true
            }
            Err(e) => {
                eprintln!(
                    "Warning: Tests did not pass ({}). Run `{} test` inside the project to \
                     investigate.",
                    e,
                    pm.cmd()
                );
    ...
    if !tests_passed {
        println!("  {} test     # re-run the failing tests", pm.cmd());
    }
    println!("  {} dev      # start the indexer locally", pm.cmd());
