...
pub struct StartArgs {
    ...
    ///per-chain and `ENVIO_PG_MAX_CONNECTIONS` is at least 4 — two per process, for two processes.
    ///A schema with an entity shared across chains, or a smaller budget, runs in one process as
    ///it always has.
