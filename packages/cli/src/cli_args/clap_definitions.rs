...
pub struct StartArgs {
    ...
    ///
    ///Only needed to place the chains yourself, across machines or under your own process
    ///manager. A plain `envio start` already splits a per-chain schema across processes and
    ///manages them for you.
    ///Requires a schema whose entities are all per-chain, created for every chain by
    ///`envio local db-migrate up` before any process starts. Assign each configured chain to
    ///exactly one process, and give each its own `ENVIO_INDEXER_PORT`. Each process reports its
    ///own chains ready as they catch up, independently of the others.
