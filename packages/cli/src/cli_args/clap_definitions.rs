...
pub struct StartArgs {
    ...
    ///processes, and manages those processes for you, whenever the schema's entities are all
    ///per-chain. `ENVIO_PG_MAX_CONNECTIONS` is the budget for the whole run and buys one process
    ///per two connections, so raising it from its default of 2 is what splits a run. A schema
    ///with an entity shared across chains, or a budget under 4, runs in one process as it always
    ///has.
