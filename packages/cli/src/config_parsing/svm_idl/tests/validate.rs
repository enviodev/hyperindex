...
fn records_each_defect_against_what_carries_it() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "instruction shadowed by one set aside for its args: transferChecked set aside: idl.json:4:18: args.amount: `coption` is not Borsh-compatible and cannot be decoded",
            "instruction whose discriminator cannot be read: sub set aside: idl.json:3:18: discriminator: expected a byte (0-255), got 999",
            "a sibling with a discriminator survives one without: accepted",
