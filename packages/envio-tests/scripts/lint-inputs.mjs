...
// It is also unnecessary: a resolve with no matching call pending parks its
...
for (const path of resFiles(testRoot)) {
  ...
  lines.forEach((line, index) => {
    ...
    const match = next < lines.length && lines[next].match(resolveCall);
...
if (violations.length > 0) {
  console.error(
    [
      ...
      ...violations,
      "",
      "Delete the delay. With nothing pending the resolve parks its answer for the next",
