...
// resolve "latest" against, so it's treated as unconstrained (0) — any
// explicit startBlock a test passes to process() is accepted, and the static
// `indexer.chains[N].startBlock` getter reports 0 rather than throwing.
...
indexer.onEvent({ contract: "NftFactory", event: "SimpleNftCreated" }, async ({ event, context }) => {
  context.Seen.set({ id: \`\${event.block.number}\`, block: event.block.number });
});
`,
  ~test=`
...
describe("start_block: latest in the test indexer", () => {
  ...
  it("accepts any explicit startBlock, since latest is treated as unconstrained", async (t) => {
    const indexer = createTestIndexer();
    ...
    t.expect((await indexer.Seen.getAll()).map((s) => s.block)).toEqual([550]);
  });
});
`,
)
