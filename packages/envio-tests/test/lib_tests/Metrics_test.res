...
`,
  ...
  it("Escapes both the effect and the chainId label values", t => {
    ...
    t.expect(
      ...
      )->String.includes(`envio_effect_call_total{effect="a\\",b=c",chainId="d\\"e"} 2`),
    ).toBe(true)
  })
  ...
  it("Renders every metric family from a fully populated snapshot", t => {
    let metrics: Metrics.t = {
      ...
      effects: [
        {
          effect: "getMetadata",
          chainId: Internal.chainScopeToString(CrossChain),
          callSeconds: 8.4,
...
envio_effect_call_seconds{effect="getMetadata",chainId="crossChain"} 8.4
...
envio_effect_call_seconds_total{effect="getMetadata",chainId="crossChain"} 20.9
...
envio_effect_call_total{effect="getMetadata",chainId="crossChain"} 300
...
envio_effect_active_calls{effect="getMetadata",chainId="crossChain"} 1

# HELP envio_effect_cache The number of items in the effect cache.
# TYPE envio_effect_cache gauge
envio_effect_cache{effect="getMetadata",chainId="crossChain"} 128
...
envio_effect_queue{effect="getMetadata",chainId="crossChain"} 4
