...
describe("Throttler", () => {
  Async.itWithOptions("Schedules and throttles functions as expected", {retry: 3}, async t => {
    let throttler = Throttler.make(~intervalMillis=100, ~logger=Logger.quiet(), ~params={"test": "Throttler"}->Internal.toLogParams)
    let actionsCalled = []
  ...
  Async.itWithOptions("Does not run until previous task is finished", {retry: 3}, async t => {
    let throttler = Throttler.make(~intervalMillis=50, ~logger=Logger.quiet(), ~params={"test": "Throttler"}->Internal.toLogParams)
    let actionsCalled = []
    throttler->Throttler.schedule(
      async () => {
