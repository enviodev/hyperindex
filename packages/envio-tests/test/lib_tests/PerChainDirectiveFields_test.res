...
`,
    ...
    ).toBe(`Config parse error: Invalid \`clickhouse.orderBy\` on \`Transfer\`: \`chain_id\` is not a column of the entity.
...
`,
    ...
    ).toBe(`Config parse error: Invalid \`clickhouse.orderBy\` on \`Transfer\`: \`chainId\` is not a column of the entity.
...
`,
    ...
    ).toBe(`Config parse error: Invalid \`clickhouse.orderBy\` on \`Transfer\`: \`blockNumber\` is not a column of the entity.
...
`,
    ...
    ).toBe(`Config parse error: Invalid \`@index\` on \`Transfer\`: \`chainId\` is already part of the primary key, and on its own it has one value per chain — too few to index by.
...
`,
    ...
    ).toBe(`Config parse error: Invalid \`@index\` on \`Transfer\`: \`chainId\` is not a column of the entity.
  envio only appends a chain column to per-chain entities, and \`Transfer\` is \`@crossChain\`. Drop \`@crossChain\`, or declare a \`chainId\` field yourself.`)
  })
...
`,
    ...
    ).toBe(`Config parse error: Invalid \`@index\` on \`Transfer\`: \`db_write_timestamp\` is not a column of the entity.
  Available columns: \`timestamp\`, \`chainId\`.`)
  })
})
