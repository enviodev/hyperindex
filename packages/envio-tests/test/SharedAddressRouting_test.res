...
//
// Routing lives inside the native addon, so a local HyperSync server is the
// only place a test can see what one log does when two contracts index its
// emitter. The query has to carry both contracts' selections, and a log both
// declare has to reach both registrations.
...
let makeSource = (~url) => {
  ...
  // One partition over every address the chain indexes, the way a merged
  // partition carries both contracts' registrations of the shared address.
  let addressSet =
    addressStore
...
// `topics` are the indexed params after topic0, so their count is part of what
// a registration matches on.
...
describe("An address indexed by two contracts", () => {
  Async.it("asks HyperSync for both contracts' selections", async t => {
    ...
    // One log selection per contract, each with its own topic0 set — the
    // shared address appears in both.
    t.expect(
      queries
      ->Array.map(query =>
        query
  ...
  Async.it("routes one log to every contract that declares it", async t => {
    ...
    // The shared Transfer fans out to both contracts, in registration order;
    // the Deposit reaches only the contract that declares it, and the
    // Vault-only emitter never reaches Token.
