...
module Registration = {
  ...
  type input = {
    ...
    // Earliest block this registration accepts; 0 is unrestricted. The address
    // store's start block is contract-wide, so it can't hold one registration
    // back when a sibling declares no start block.
    startBlock: int,
