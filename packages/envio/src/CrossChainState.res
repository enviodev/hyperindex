...
// Items reserved by every in-flight query across every chain. Credited back to
// each chain's own target (so its in-flight work counts toward the reach) while
// drawn against the indexer-wide free buffer budget.
let totalReserved = (crossChainState: t) => {
  let total = ref(0.)
  for i in 0 to crossChainState.chainIds->Array.length - 1 {
    let cs = crossChainState->getChainState(crossChainState.chainIds->Array.getUnsafe(i))
...
// Dispatch a fetch tick across the whole indexer, sequentially, most-behind first.
//
// The indexer keeps ~targetBufferSize items ready to process. The free budget for
// this tick is that target minus what's already ready across every chain minus
// what in-flight queries have reserved. Chains are visited furthest-behind first
// (priorityOrder); each takes the share it can use and passes the rest on. A
// chain's target is the remaining budget plus its own in-flight reservation (so
// work already dispatched for it counts toward the reach, not against it), which
// ChainState.getQueries turns into a block reach via the chain-level density and
// sizes each query's itemsTarget from the owning partition's own density. Only
// when a chain can't spend its whole share — it reached the head — does budget
// reach the next chain, so during backfill the most-behind chain gets it all.
let checkAndFetch = async (
  ...
  let rangeBudget = ref(
    Pervasives.max(
      ...
      crossChainState->totalReserved->Float.toInt,
    ),
  )

  let promises = []
  let priorityChains = crossChainState->priorityOrder
  for i in 0 to priorityChains->Array.length - 1 {
    let cs = priorityChains->Array.getUnsafe(i)
    let chainId = (cs->ChainState.chainConfig).id
    // Credit the chain's own reservation back to its target so the reach counts
    // in-flight work; the budget it can actually spend stays the free space.
    let chainTargetItems = rangeBudget.contents + cs->ChainState.reservedSize->Float.toInt
    let action = cs->ChainState.getQueries(~chainTargetItems, ~budget=rangeBudget.contents)
    switch action {
    | Ready(queries) =>
      let consumed = ref(0)
      let logged = Dict.make()
      queries->Array.forEach((query: FetchState.query) => {
        consumed := consumed.contents + query.itemsTarget
        logged->Dict.set(
          query.partitionId,
          {
      ...
      rangeBudget := Pervasives.max(0, rangeBudget.contents - consumed.contents)
      Logging.trace({
      ...
      promises
      ->Array.push(dispatchChain(~chain=ChainMap.Chain.makeUnsafe(~chainId), ~action))
      ->ignore
    | WaitingForNewBlock =>
      promises
      ->Array.push(dispatchChain(~chain=ChainMap.Chain.makeUnsafe(~chainId), ~action))
