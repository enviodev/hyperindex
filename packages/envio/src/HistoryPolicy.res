...
let decide = (config: Config.t, ~inThreshold) =>
  if config.shouldSaveFullHistory {
    ...
    inThreshold() ? Keep : Skip
  }

let anyChainInThreshold = (config: Config.t, ~inThreshold: ChainId.t => bool) =>
  () => config.chainMap->ChainMap.keys->Array.some(inThreshold)
...
let forBatch = (config: Config.t, ~inThreshold: ChainId.t => bool) =>
  config->decide(~inThreshold=config->anyChainInThreshold(~inThreshold))
...
let forScope = (config: Config.t, ~scope: Internal.chainScope, ~inThreshold: ChainId.t => bool) =>
  switch (config.checkpointSequence, scope) {
  | (Global, _) => config->forBatch(~inThreshold)
  | (PerChain, Chain(chainId)) =>
    config->decide(~inThreshold=() =>
      inThreshold(chainId) && (config.chainMap->ChainMap.get(chainId)).maxReorgDepth > 0
    )
  | (PerChain, CrossChain) =>
